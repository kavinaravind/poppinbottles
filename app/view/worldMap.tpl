<div id="bodyWrapper" class = "bodyMap">
	...
	var colors = d3.scale.category10();
		...
		function getMap() {
			...
			//       },
			//       function(data) {
			//       	$("#mapProduct form img").show();
			//       },
		 //    			'json'
		 //    	);
		$.ajax({
					url: "<?= BASE_URL ?>"+ "/products/wineMap/",
					type: "GET",
		...
		$(document).on("click", ".showProduct", function() {
			...
			//$("#mapProduct form img").attr("src", "");
			//var timestamp = new Date().getTime();
			$("#mapProduct form img").attr("src", "<?= BASE_URL ?>/public/img/"+imgURL);
			// $("#mapProduct form img").one("load", function() {
			//   // do stuff
			//   $("#mapProduct form img").attr("src", "<?= BASE_URL ?>/public/img/"+imgURL);
			// }).each(function() {
			...
			//$("#mapProduct").load(location.href + "#mapProduct>*", "");
			return true;
		});
